...

{
  ...
  struct Exp_t *left;  // can be Exp_Int or Exp_Sum
...
{
  // TODO();
  struct List_t *reverse_list = NULL;
  struct List_t *p = list;
  // reverse order
  while (p != NULL) {
    reverse_list = List_new(p->instr, reverse_list);
    p = p->next;
  }
  ...
  while (p != NULL) {
    if (p->instr->kind == STACK_PUSH) {
      struct Stack_Push *push_instr = (struct Stack_Push *)(p->instr);
      printf("push %d\n", push_instr->i);
    } else {
      struct Stack_Add *add_instr = (struct Stack_Add *)(p->instr);
      printf("add\n");
    }
    p = p->next;
  }
